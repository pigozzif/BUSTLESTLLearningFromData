package it.units.malelab.LocalSearch.sampler;
...
public class LatinHypercubeSampler implements GridSampler {
	 ...
	 *            {@code subdivisions^dim}.
	 */
	public LatinHypercubeSampler(int seed, int subdivisions) {
	...
	private double[][] sampleOrthogonal(int dim, int subdivisions,
		...
		while (sampleIndex < samples.length) {
			...
			for (int j = 0; j < Ns; j++) {
				...
				for (int i = 0; i < dim; i++) {
                    ...
                    System.arraycopy(indexVectors[i][pj[i]], 1, indexVectors[i][pj[i]], 0, indexVectors[i][pj[i]].length - 1);
	 ...
	 private int[] randperm(final int n) {
