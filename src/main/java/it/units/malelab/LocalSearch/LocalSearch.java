package it.units.malelab.LocalSearch;
...
import it.units.malelab.LocalSearch.gpOptimisation.GPOptimisation;
import it.units.malelab.LocalSearch.gpOptimisation.GpoOptions;
...
import it.units.malelab.LocalSearch.sampler.Parameter;
            ...
            public double[][] sample(int n, double[] lbounds, double[] ubounds) {
                ...
                for (int i = 0; i < n; i++) {
                    for (int j = 0; j < numBounds; j += 2) {
                        ...
                        res[i][j + 1] = res[i][j] + (Math.max(Math.random(), 0.00000001) * (ubounds[j + 1] - res[i][j]));
                    }
